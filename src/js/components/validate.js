...
let colorValue = style.getPropertyValue('--color-accent');
...
let productsFormData = null; // все данные о продуктах, которые попадают в модалку.
...
let validateForms = function(selector, rules, messages, successModal, yaGoal) {
    ...
    colorWrong: colorValue,
      ...
      if (form.classList.contains('cart-modal__form')) {
        ...
        document.querySelectorAll('.cart-modal-order__list .mini-cart__item').forEach((el, index) => {
          let title = el.querySelector('.mini-product__title').textContent;
          let price = el.querySelector('.mini-product__price').textContent;
          productsFormData.append(`product-${index + 1}`, `${title}, ${price}`);
        });
