                ...
                <article class="product">
                  <div class="product__image">
                    ...
                    <div class="product__btns">
                      <button class="product__btn" data-graph-path="prod-modal" data-id="${item.id}" aria-label="Показать информацию о товаре">
                        <svg>
                          <use xlink:href="img/sprite.svg#eye"></use>
                        </svg>
                      </button>
                      <button class="product__btn add-to-cart-btn" data-id="${item.id}" aria-label="Добавить товар в корзину">
                        <svg>
    ...
    fetch('data/data.json')
      ...
      .then((data) => {
        ...
        for (let dataItem of data) {
          if (dataItem.id == id) {
            ...
            const slides = dataItem.gallery.map((image, index) => {
              return `
            ...
            const preview = dataItem.gallery.map((image, index) => {
              return `
                <div class="modal-preview__item ${index === 0 ? 'modal-preview__item--active' : ''}"
            ...
            Object.keys(dataItem.chars).forEach(function eachKey(key) {
              charsItems += `
                <p class="prod-bottom__descr prod-chars__item">${key}: ${dataItem.chars[key]}</p>
              `;
            });
      ...
      .then(() => {
        ...
        prodSlider.on('slideChangeTransitionEnd', function () {
          let index = document.querySelector('.swiper-slide-active').dataset.index;
          document.querySelectorAll('.modal-preview__item').forEach(el => {
            el.classList.remove('modal-preview__item--active')
          });
          document.querySelector(`.modal-preview__item[data-index="${index}"]`)
